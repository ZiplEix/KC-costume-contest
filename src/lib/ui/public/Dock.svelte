<script lang="ts">
    ...
    import { publicDockStore, type PublicDockName } from "$lib/stores/dock";
    ...
    onMount(() => {
        ...
        if (currentPath === "/") {
            publicDockStore.set("home");
        } else if (currentPath === "/rules") {
            publicDockStore.set("rules");
        } else {
            publicDockStore.set("home");
        }
    });

    function onClick(dockTab: PublicDockName) {
        publicDockStore.set(dockTab);
...
<div class="dock flex justify-around">
    <button on:click={() => onClick("home")} class:dock-active={$publicDockStore === "home"}>
    ...
    <button on:click={() => onClick("votes")} class:dock-active={$publicDockStore === "votes"}>
    ...
    <button on:click={() => onClick("rules")} class:dock-active={$publicDockStore === "rules"}>
